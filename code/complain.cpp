...
this->complainee_id = id;
	...
	string rr;
	sss>>rr;
	this->ComplainID = "CP"+id+"DT"+this->DateR+"|"+rr;
};
...
void ComplainList::updateComplainStatus(){ // function for updating status of  a complain
	system("clear");
...
vector<Complain> ComplainList::findComplain_by_RecipientID(string Rid){     // find complain by recipient id
	list<Complain>::iterator it =All_complains.begin();
	vector<Complain> vec;	
	while(it!=All_complains.end()){
...
vector<string> ComplainList::extract(string info){  // This function interprets the string and creates information
int i=0;
	vector<string> vec;
	while(i<=info.size()){
	string h;
